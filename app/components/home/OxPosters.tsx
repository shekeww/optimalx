...
import { CONTENT_CARDS, contentHref } from '../../content/posters';
import { ContentPosterCard } from './PosterCard';
 ...
 * step every arrow press moves, same convention `OxBrands`/`OxCategoryRail`
 * use. The rail's own cue steps one card at a time instead (`nudgeRail`), so
 * it reaches the last card on a phone too.
 */
const STEP_AT_DESKTOP = 3;

/** The five content cards with every destination and every string resolved. */
function useContentCards() {
...
/**
 * The "اكتشف أكثر" carousel: the five content cards (a type, two goals, the
 * advisory and the branch), each the theme's own photograph, title, line and
 * angled arrow (restored from 4657b89, docs/build/progress/S8a.md).
 *
 * Owner review 2026-09-25: the six offer posters that used to alternate with
 * these cards now have their own carousel inside "العروض"
 * (`OxProductsSecondary`), and this block moved down the page, after the
 * eight category rails and right before the advisory band
 * (`HOME_BLOCK_PATHS`). The cards, the rail and the header are unchanged.
 *
 * The cards sit on the shared rail primitive (owner review 2026-09-23 late
 * night, item 2), sized so 1.15 of a card shows at 390 (a peek of the next
 * one), 2 at 768, 3 at 1024, 4 at 1440 (the width arithmetic in
 * `_b2-home.scss` §17.2). A scroller that ends flush at the container's edge
 * looks finished when it is not, and every retail app this market has
 * learned shows the next card instead.
 *
 ...
 * asked to drag. No native scrollbar draws under the row.
 *
 * The row bleeds: the track sits outside `.ox-container` and runs off the
 * trailing edge of the screen, which is what makes the partial card read as
 * "there is more that way" rather than "this box is clipped". The header
 * (title and, from 1024, the arrow pair) stays inside the container above
 * it, same split `OxBrands` uses for its own full-bleed background.
 *
 * A content card links its literal route or its taxonomy node through the
 * same `useTaxonomyLinks()` every goal and type link uses (`contentHref()`).
 * `label`/`label_en` override the section's own title, per the active
 * locale.
 */
  ...
  return (
    <section className="ox-posters" data-testid="ox-posters">
      <div className="ox-container">
        <SectionHeader
          // "اكتشف أكثر" over a subline naming what the cards lead to
          // (owner item 2026-09-24, S8a; the title is distinct from the
          // offers section's own, UX-2026-09-24 P0-11).
          title={label}
      ...
      {/* The track sits outside the container so it can run off the trailing
          ...
          against the same container the header above uses. `.ox-rail`
          (`_rail.scss`) supplies the no-scrollbar track and the chevron cue;
          the reveal stagger targets the track itself (`revealRef`). */}
