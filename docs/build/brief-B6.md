...
## Addendum (2026-09-19, from the Chrome handback)
- The engine mounts `CartContextProvider` only on the cart route, so `useCartContext` is null elsewhere. The header cart button already shows the SDK-driven count from `salla-cart-summary` (Chrome's `useCartCountPill`). B6 owns a site-wide cart count source: subscribe to the SDK cart events (`salla.event.cart.on...` or `salla.cart.event`, verify the name in the CDN SDK typings or by reading `window.salla` at runtime in the preview) in a small `useCartCount` hook under `app/components/commerce/`, and expose it through a tiny store (module singleton with `useSyncExternalStore`) that Chrome's `BottomTabBar` can read; then message the conductor with the hook path so the tab bar is wired between waves (you do not edit `app/components/layout/*`).
- Locale values use i18next double braces (`{{threshold}}`), never single.
- `twilight.json` already carries `show_newsletter`, `reply_sla_hours`, `consultation_credit_note` and the branch settings; read them defensively.
