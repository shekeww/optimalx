...
- Branch `docs/engine-defect-and-spec-trueup`, HEAD `9d8a70e`, pushed. Owner tooling auto-commits staged work with the message `commit_editmsg`; agents never stage or commit.
...
- Store 1888890798 (optimalx.com.sa): 47 mock products live (ids in `docs/build/salla-ids.json` and the `salla_id` column of `docs/build/research/optimalx-catalogue.csv`), 0 categories, 0 brands, single language (`is_multilingual: false`, Arabic at root, no `/ar` prefix). The live domain runs the Raed theme with an injected skin; the React theme replaces it after Partners review (theme id 10165682, `docs/DEPLOY.md`).
