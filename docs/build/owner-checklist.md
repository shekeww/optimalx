...
After deployment (your ruling, 2026-09-25): shipping zones and markets, the payment provider, the refund policy text, the registration values (CR, VAT, Maroof) and the admin store description are yours to set up after the store goes live; the theme already reads each of them from settings and hides what is empty.

| Item | What the theme does today | What you decide or provide |
|---|---|---|
| Store phone and email | Done 2026-09-25: the theme settings store_phone (+966553524524) and contact_email (contact@optimalx.com.sa) feed the contact page, the FAQ contact row, the branch and services pages, the drawer and the structured data; the admin email (your account address) is never shown to shoppers, not even as a fallback | Nothing: you keep the account address as the admin email (your ruling, 2026-09-25) |
...
| Hero poster (frame 4) | Approved by you on 2026-09-25 as drawn, its lines included: banned words are judged by intent and impression, not literally (recorded in FINAL-claims-source.md). Frame 4 of the home carousel on desktop and phone, in the order 1 the bars and chips, 2 the home photo, 3 the athlete, 4 the poster, 5 the shaker scene; its text is Arabic, so the English home shows the other four | Nothing |
| Payment marks | Added 2026-09-25: the methods show under the buy buttons, in the cart summary and in the footer, all eight ticked by default. The artwork is Shopify's own payment icon set: every mark is a 38 x 24 colour tile, tamara's tile is its black symbol rather than the wordmark, and the stc mark reads "stc pay". The design asked for a quiet one-colour row on the product page, which needs each provider's own one-colour files | Untick every method your checkout does not offer (Theme settings, Payment methods). Then choose: keep Shopify's colour tiles, or send the official one-colour and wordmark files from the mada, Visa, Mastercard, STC Bank, tabby and tamara merchant kits (never redrawn). Also confirm "stc pay" or "STC Bank" |
