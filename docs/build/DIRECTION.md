...
**BuyRow** (product/BuyZone). Anatomy: quantity input at the start (44 tall, minus, value, plus, 120 wide), add to cart primary 48 filling the rest, then a full-width secondary "اشتري الآن" 48 under the row. Sizes: 12 gap; the row is the anchor the sticky bar watches. States: add button default, hover, pressed, loading (engine loader restyled), confirmed (label swaps to a tick plus "أضيف" for --dur-confirm then returns), disabled when required options are missing (the button stays enabled but scrolls to and highlights the missing group; a truly disabled state exists only for hidden or out products). RTL: quantity at the right. Wraps: SallaQuantityInput, SallaAddProductButton, SallaQuickBuy (buy now).
