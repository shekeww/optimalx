...
/**
 * The product TYPE on the card's facts line (owner item 2026-09-24, S8a).
 * Four sources in order, each mapped to one of the ten root types: the
 * product's own `category` when the API carries it, the category listing the
 * card renders in, the theme's own SKU membership (`taxonomy.json`), then a
 * keyword table over the name that answers only on an unambiguous match.
 * The rule every test here holds: a wrong type never prints; nothing does.
 */
...
describe('the ten root types', () => {
  ...
  it('each carry a short card label in both locales, sized for the two-up phone card (S8a)', () => {
    ...
    // pushed the servings behind the ellipsis at 390; the card label is the
...
describe('productTypeOf, the four sources in order', () => {
  const whey = byName('جولد ستاندرد 100% واي بروتين - اوبتيموم نيوترشن');

  it('prefers the API category', () => {
    const product = { ...whey, category: { name: 'كرياتين', url: '/creatine/c9002' } };
    expect(productTypeOf(product, { categorySlug: 'protein' })).toBe('creatine');
  });

  it('then the listing the card renders in', () => {
    expect(productTypeOf({ ...whey, category: undefined }, { categorySlug: 'omega-3' })).toBe('omega_3');
  });

  it('then the membership, then the name', () => {
    expect(productTypeOf({ ...whey, category: undefined })).toBe('protein');
    expect(productTypeOf({ id: 1, sku: null, name: 'كرياتين مونوهيدرات - ثورن' })).toBe('creatine');
  ...
  it('falls through a goal listing and an unmapped category to the next source', () => {
    ...
    expect(productTypeOf(product, { categorySlug: 'goal-energy' })).toBe('protein');
  });
});
